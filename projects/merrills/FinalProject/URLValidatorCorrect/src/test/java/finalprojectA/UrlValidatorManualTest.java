...
public class UrlValidatorManualTest extends TestCase {
  ...
  @Test 
   public void testManualTest() throws Throwable {
	   ...
	   assertTrue(validator.isValid("http://userid:password@example.com/")); //ERROR
	   //assertTrue(validator.isValid("http:142.42.1.1/")); //ERROR Actual IsValid say
	   //not ok
	   ...
	   //assertFalse(validator.isValid("ftps://foo.bar")); //Actual isValid says valid
	   assertFalse(validator.isValid("http://-error-.invalid"));
	   //assertFalse(validator.isValid("http://0.0.0.0")); //Actual isValid says valid
